<div>
    <!-- Edit Modal -->
    <x-dialog-modal wire:model.live="modalEditSick">
        <x-slot name="title">
        ...
        <x-slot name="footer">
            <x-secondary-button wire:click="$toggle('modalEditSick')" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-secondary-button>

            <x-button class="ms-3" wire:click="updateSick" wire:loading.attr="disabled">
                {{ __('Proceed') }}
            </x-button>
        </x-slot>
    </x-dialog-modal>
    ...
    <!-- Delete Modal -->
    <x-dialog-modal wire:model.live="modalDeleteSick">
        <x-slot name="title">
            {{ __('Delete Sick Leave to an Employee') }}
        </x-slot>

        <x-slot name="content">
            {{ __('Are you sure you want to delete this employee Sick Leave? This action cannot be undone.') }}
        </x-slot>

        <x-slot name="footer">
            <x-secondary-button wire:click="$toggle('modalDeleteSick')" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-secondary-button>

            <x-button class="ms-3" wire:click="destroySick" wire:loading.attr="disabled">
                {{ __('Delete') }}
            </x-button>
        </x-slot>
    </x-dialog-modal>
